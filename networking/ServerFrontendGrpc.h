...
class ServerFrontendGrpc
{
	class ConnectionInterfaceGrpc final : public ConnectionInterface
	{
	public:
		explicit ConnectionInterfaceGrpc(ServerFrontendGrpc* pFrontend);
		~ConnectionInterfaceGrpc() override = default;
		...
		void SendEventToClient(
			...
			const ConnectionContext* pSenderForLogging) override;
		void ForceDisconnect(
			const char* pErrorMessage,
			const std::shared_ptr<ConnectionContext>& pClient,
			bool pRemovedFromTable) override;

		ServerFrontendGrpc* const Frontend; // Frontend keeps a reference to us, thus we're guaranteed to be alive as long as frontend is.
...
public:
	ServerFrontendGrpc(const char* pListeningEndpoint, const grpc::SslServerCredentialsOptions* pCredentialsOptions, std::shared_ptr<evtc_rpc_server>&& pServer);
	~ServerFrontendGrpc() = default;
...
private:
#endif
	std::shared_ptr<ConnectionContext> MakeConnectionContext();
	...
	std::shared_ptr<evtc_rpc_server> mServer;
